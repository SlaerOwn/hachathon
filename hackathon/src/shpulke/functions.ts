...
function spawnEntity(shape: GLTFShape, x: number, y: number, z: number, rotation?: Quaternion) {
    ...
    entity.addComponent(new Transform({ position: new Vector3(x + game._x, y, z + game._z), rotation }))
...
function spawnDoor(x: number, y: number, z: number, rotation?: Quaternion) {
    ...
    door.addComponent(new GLTFShape("models/shpulke/door.glb"))
    door.addComponent(new Transform({ position: new Vector3(x + game._x, y, z + game._z), rotation }))
...
function spawnInvisibleDoor(x: number, y: number, z: number, rotation?: Quaternion) {
    const door = new Entity()
    const shape = new GLTFShape("models/shpulke/door.glb")
...
function invisibleOff2(entity:Entity) {
...
export { spawnEntity, spawnDoor, spawnInvisibleEntity, spawnInvisibleDoor, invisibleOff, invisibleOn, invisibleOff2 }
